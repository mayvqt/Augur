...
Augur reads `config.json`. `AUGUR_CONFIG` or `-config` can select another file.
...
The optional HTTP server provides `/healthz`, `/readyz`, and `/metrics`. It is
disabled by default. Port `0` selects an unused local port.
